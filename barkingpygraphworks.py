...
curve = plot_widget.plot(pen='b')  # Set the initial color to blue

# Add a horizontal line to represent the threshold
threshold_line = pg.InfiniteLine(pos=threshold, angle=0, movable=False)
...
def audio_callback(indata, frames, time, status):
    ...
    if any(indata > threshold):
        if not recording_started:
            ...
            # Change the color to red when the threshold is reached
